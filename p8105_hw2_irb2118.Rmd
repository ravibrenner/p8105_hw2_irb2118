...
The average viewership was `r viewers_df |> filter(series == 1) |> summarize(mean(viewers,na.rm = TRUE))` in season 1 and `r viewers_df |> filter(series == 5) |> summarize(mean(viewers,na.rm = TRUE))` in season 5 (excluding episodes with missing viewership data). Although no explicitly stated, these viewership numbers likely represent either some multiple of viewers (i.e., time 1,000,000), or some other TV rating scale that may take live and streaming viewership into account. In either case, the viewership in season 5 is much larger than that of season 1.

# Conclusion
This homeworks assignment showcased various techniques related to importing, cleaning, tidying, and manipulating data using R and Rmarkdown.
