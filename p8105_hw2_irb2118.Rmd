...
With these 3 trash wheel datasets combined, there are `r nrow(trash_wheels)` rows (`r nrow(mr_trash_wheel)` for Mr. Trash Wheel, `r nrow(prof_trash_wheel)` for Professor Trash Wheel, and `r nrow(gwyn_trash_wheel)` for Gwynnda Trash Wheel) and `r ncol(trash_wheels)` columns. Observations range from `r min(pull(trash_wheels, date))` to `r max(pull(trash_wheels,date))`, and there were `r sum(pull(trash_wheels,cigarette_butts))` cigarette butts collected in that period!
